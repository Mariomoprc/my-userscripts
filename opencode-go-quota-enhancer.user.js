...
(function () {
  ...
  console.log(TAG, 'v1.4.1 loaded, pathname:', location.pathname);
  var MODEL_META = {
  ...
  function toast(text, color) { var d = document.createElement('div'); d.style.cssText = 'position:fixed;top:10px;right:10px;z-index:2147483647;background:rgba(0,0,0,.9);color:' + (color || '#0f0') + ';padding:10px 14px;border-radius:8px;font-size:12px;font-family:monospace;max-width:360px;line-height:1.5;box-shadow:0 2px 8px rgba(0,0,0,.3);'; d.textContent = text; document.body.appendChild(d); setTimeout(function () { if (d.parentNode) d.remove(); }, 3500); }
  ...
  function computeScore(model) { var reqScore = Math.min(model.req5h / 45300 * 100, 100); var ctxScore = Math.min(Math.log10(model.context || 128000) / Math.log10(1000000) * 100, 100); var priceScore = model.input > 0 ? Math.min((1 - model.input / 3.0) * 100, 100) : 50; var modScore = Math.min((model.modalities || []).length / 3 * 100, 100); return Math.round(reqScore * 0.35 + ctxScore * 0.25 + priceScore * 0.25 + modScore * 0.15); }
  function parseTables(html) { var tables = html.match(/<table[\s\S]*?<\/table>/g) || []; function rows(tableHtml) { var result = []; var trRe = /<tr>([\s\S]*?)<\/tr>/g; var m; var first = true; while ((m = trRe.exec(tableHtml)) !== null) { if (first) { first = false; continue; } var cells = []; var tdRe = /<t[dh]>([\s\S]*?)<\/t[dh]>/g; var c; while ((c = tdRe.exec(m[1])) !== null) { cells.push(c[1].replace(/<[^>]+>/g, '').trim()); } if (cells.length > 0) result.push(cells); } return result; } var requests = [], prices = [], endpoints = []; tables.forEach(function (t) { var header = (t.match(/<thead>([\s\S]*?)<\/thead>/) || ['', ''])[1]; var ths = (header.match(/<th[^>]*>([\s\S]*?)<\/th>/g) || []).map(function (h) { return h.replace(/<[^>]+>/g, '').trim().toLowerCase(); }); var joined = ths.join('|'); if (joined.indexOf('requests per 5') !== -1) requests = rows(t); else if (joined.indexOf('input') !== -1 && joined.indexOf('output') !== -1 && joined.indexOf('usage') !== -1) prices = rows(t); else if (joined.indexOf('model id') !== -1) endpoints = rows(t); }); return { requests: requests, prices: prices, endpoints: endpoints }; }
  function mergeData(tables, apiModels) { var map = {}; tables.requests.forEach(function (r) { var name = r[0] || ''; var n = norm(name); if (!n) return; map[n] = { name: name, modelId: '', req5h: parseNum(r[1]), reqWeek: parseNum(r[2]), reqMonth: parseNum(r[3]), input: 0, output: 0, usage: 0, context: 128000, modalities: ['text'], reasoning: false, score: 0 }; }); tables.prices.forEach(function (r) { var rawName = r[0] || ''; var baseName = rawName.replace(/\s*[\(\uFF08].*$/, '').trim(); var n = norm(baseName); var isUpperTier = rawName.indexOf('>') !== -1; if (!n || !map[n]) return; if (isUpperTier && map[n].input > 0) return; map[n].input = parseNum(r[1]); map[n].output = parseNum(r[2]); map[n].usage = parseNum(r[5]); }); tables.endpoints.forEach(function (r) { var name = r[0] || ''; var n = norm(name); if (!n || !map[n]) return; map[n].modelId = r[1] || ''; }); Object.keys(map).forEach(function (k) { var m = map[k]; var meta = MODEL_META[m.modelId]; if (meta) { m.context = meta.context; m.modalities = meta.modalities; m.reasoning = meta.reasoning; } m.score = computeScore(m); }); if (apiModels && apiModels.length) { var docsIds = {}; Object.keys(map).forEach(function (k) { docsIds[map[k].modelId] = true; }); apiModels.forEach(function (m) { if (m.id && !docsIds[m.id]) { var meta = MODEL_META[m.id] || {}; var entry = { name: m.id, modelId: m.id, req5h: 0, reqWeek: 0, reqMonth: 0, input: 0, output: 0, usage: 0, context: meta.context || 128000, modalities: meta.modalities || ['text'], reasoning: meta.reasoning || false, score: 0, isNew: true }; entry.score = computeScore(entry); map[m.id] = entry; } }); } var arr = Object.keys(map).map(function (k) { return map[k]; }); arr.sort(function (a, b) { return b.score - a.score; }); return arr; }
  function usageColor(usage) { if (usage >= 60) return '#2ea043'; if (usage >= 30) return '#1f6feb'; if (usage >= 15) return '#d29922'; return '#f85149'; }
  function scoreColor(score) { if (score >= 80) return '#2ea043'; if (score >= 60) return '#1f6feb'; if (score >= 40) return '#d29922'; return '#f85149'; }
  function renderPanel(data) {
    ...
    header.innerHTML = '<div><strong style="font-size:15px;">Go \u6A21\u578B\u989D\u5EA6 \u00B7 \u7EFC\u5408\u8BC4\u5206\u699C</strong><span style="margin-left:8px;font-size:11px;opacity:0.6;">' + totalModels + ' \u4E2A\u6A21\u578B \u00B7 ' + new Date().toLocaleDateString('zh-CN') + ' \u66F4\u65B0</span></div><div><button id="oc-go-refresh" style="padding:4px 10px;margin-right:6px;cursor:pointer;border:1px solid #555;border-radius:4px;background:#222;color:#ccc;font-size:11px;">\u5237\u65B0</button><button id="oc-go-toggle" style="padding:4px 10px;margin-right:6px;cursor:pointer;border:1px solid #555;border-radius:4px;background:#222;color:#ccc;font-size:11px;">\u6298\u53E0</button><button id="oc-go-close" style="padding:4px 10px;cursor:pointer;border:1px solid #555;border-radius:4px;background:#222;color:#ccc;font-size:11px;">\u2715</button></div>';
    ...
    stats.innerHTML = '<span style="background:rgba(46,160,67,.15);padding:3px 8px;border-radius:4px;color:#2ea043;">Top: ' + (topModel.name || '-') + ' (' + (topModel.score || 0) + '\u5206)</span><span style="background:rgba(31,111,235,.15);padding:3px 8px;border-radius:4px;color:#1f6feb;">\u226580\u5206: ' + highScore + ' \u4E2A</span><span style="background:rgba(210,153,34,.15);padding:3px 8px;border-radius:4px;color:#d29922;">\u8BC6\u56FE: ' + imgModels + ' \u4E2A</span>';
    panel.appendChild(stats);
    var controls = document.createElement('div');
    controls.style.cssText = 'display:flex;gap:8px;margin-bottom:10px;align-items:center;';
    controls.innerHTML = '<input id="oc-go-search" type="text" placeholder="\u641C\u7D22\u6A21\u578B..." style="flex:1;max-width:180px;padding:5px 8px;border:1px solid #555;border-radius:4px;background:#222;color:#ccc;font-size:12px;" /><select id="oc-go-sort" style="padding:5px 8px;border:1px solid #555;border-radius:4px;background:#222;color:#ccc;font-size:12px;"><option value="score">\u6309\u7EFC\u5408\u8BC4\u5206</option><option value="req5h">\u6309 5h \u6B21\u6570</option><option value="context">\u6309\u4E0A\u4E0B\u6587\u5927\u5C0F</option><option value="input">\u6309\u8F93\u5165\u4EF7</option><option value="usage">\u6309\u500D\u6570</option></select>';
    ...
    function renderTable(items) {
      ...
      t.innerHTML = '<thead><tr style="border-bottom:1px solid #444;text-align:left;position:sticky;top:0;background:#1a1a2e;"><th style="padding:6px 6px;">\u6A21\u578B</th><th style="padding:6px 6px;text-align:center;">\u6A21\u6001</th><th style="padding:6px 6px;text-align:right;">\u4E0A\u4E0B\u6587</th><th style="padding:6px 6px;text-align:right;">5h</th><th style="padding:6px 6px;text-align:right;">\u8F93\u5165\u4EF7</th><th style="padding:6px 6px;text-align:center;">\u500D\u6570</th><th style="padding:6px 6px;">\u64CD\u4F5C</th></tr></thead>';
      ...
      items.forEach(function (d, i) {
        ...
        tr.innerHTML = '<td style="padding:6px 6px;' + nameStyle + '"' + idTip + '>' + d.name + ' <span style="color:' + scoreColor(d.score) + ';font-weight:600;font-size:11px;">' + d.score + '\u5206</span> <span style="font-size:10px;color:' + scoreColor(d.score) + ';">' + stars(d.score) + '</span>' + (d.isNew ? ' <span style="font-size:10px;color:#f85149;">NEW</span>' : '') + '</td><td style="padding:6px 6px;text-align:center;font-size:11px;">' + modalitiesText(d.modalities) + (d.reasoning ? ' <span style="color:#2ea043;">\u63A8\u7406</span>' : '') + '</td><td style="padding:6px 6px;text-align:right;font-size:11px;">' + formatContext(d.context) + '</td><td style="padding:6px 6px;text-align:right;">' + d.req5h.toLocaleString() + '</td><td style="padding:6px 6px;text-align:right;">' + (d.input ? '$' + d.input.toFixed(2) : '-') + '</td><td style="padding:6px 6px;text-align:center;"><span style="display:inline-block;padding:2px 6px;border-radius:3px;font-size:11px;font-weight:600;color:#fff;background:' + usageColor(d.usage) + ';">' + (d.usage ? d.usage + 'x' : '-') + '</span></td><td style="padding:6px 6px;"><button class="oc-go-copy" data-id="' + (d.modelId || '') + '" style="padding:2px 8px;cursor:pointer;border:1px solid #555;border-radius:3px;background:transparent;color:#aaa;font-size:11px;' + (d.modelId ? '' : 'opacity:0.3;cursor:default;') + '">\u590D\u5236</button></td>';
    ...
    footer.innerHTML = '<span style="opacity:0.5;">\u8BC4\u5206 = \u989D\u5EA635% + \u4E0A\u4E0B\u658725% + \u4EF7\u683C25% + \u6A21\u600115%</span><span style="opacity:0.5;">v1.4.1</span>';
  ...
  function injectToggleButton() {
    ...
    btn.addEventListener('click', function () { var panel = document.getElementById('oc-go-panel'); if (panel) { panel.remove(); try { localStorage.setItem(PANEL_KEY, '0'); } catch (e) {} } else { try { localStorage.setItem(PANEL_KEY, '1'); } catch (e) {} loadAndInject(false); } });
  ...
  function injectPanel(data, source) {
    ...
    try {
      ...
      panel.style.position = 'fixed'; panel.style.top = '16px'; panel.style.right = '16px'; panel.style.width = '680px'; panel.style.maxWidth = 'calc(100vw - 32px)'; panel.style.maxHeight = '85vh'; panel.style.overflowY = 'auto'; panel.style.zIndex = '2147483647'; panel.style.boxShadow = '0 4px 24px rgba(0,0,0,.5)'; panel.style.margin = '0';
