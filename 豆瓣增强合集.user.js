...
// @version      1.0.0
// @description  豆瓣增强：自动加载更多 + 剧集完结时间/更新进度（豆瓣全站）
// @author       pass
...
(function () {
  ...
  (function () {
    ...
    function getDoubanInfo() {
        ...
        var akaM = txt.match(/又名:\s*(.+)/); var aka = akaM ? akaM[1].split('/')[0].trim() : '';
        var region = regionM ? regionM[1].trim() : '';
        return { imdbId: imdbM ? imdbM[1] : '', season: seasonM ? parseInt(seasonM[1]) : null, totalEpisodes: episodeM ? parseInt(episodeM[1]) : null, premiere: premiereM ? premiereM[1] : null, engTitle: engTitle, chTitle: chTitle, aka: aka, region: region, title: title };
    }
    ...
    function calculateAiredEpisodes(premiereDate, epPerWeek, totalEpisodes){ var today=new Date(); var premiere=new Date(premiereDate); var daysSinceStart=Math.floor((today-premiere)/(1000*60*60*24)); if(daysSinceStart<0) return 0; var weeksAired=Math.floor(daysSinceStart/7); var aired=weeksAired*epPerWeek; if(daysSinceStart===0) aired=1; return Math.min(aired,totalEpisodes); }
    function getMDLSearchUrl(info){ var query=info.engTitle||info.aka||info.chTitle; if(!query) return window.location.href; return 'https://mydramalist.com/search?adv=titles&ty=68&q='+encodeURIComponent(query); }
    function showResult(data){
        ...
        else { var updateFreq=''; if(data.epPerWeek&&!data.updateDay) updateFreq=' 每周'+data.epPerWeek+'集'; else if(data.updateDay&&data.epPerWeek) updateFreq=' 每'+data.updateDay+'更新'+data.epPerWeek+'集'; else if(data.updateDay) updateFreq=' 每'+data.updateDay+'更新'; span.innerHTML='<span style="color:#66bb6a;">▶</span> <b>更新中</b> '+data.aired+'/'+data.total+updateFreq+' <span style="color:#888;">预估完结:</span> <b>'+data.endDate+'</b>'+sourceTag+link; }
    }
    ...
    function matchShow(results, info){ if(!info.premiere) return null; var year=new Date(info.premiere).getFullYear(); for(var i=0;i<results.length;i++){var s=results[i].show; if(s&&s.premiered){var showYear=new Date(s.premiered).getFullYear(); if(Math.abs(showYear-year)<=3) return s.id;}} return null; }
    function lookupByImdb(imdbId){ return fetchJSON(TVMAZE+'/lookup?imdb='+imdbId).then(function(data){ if(data&&data.id) return data.id; return null; }); }
    ...
    function main(){
        ...
        var searchPromise; if(info.imdbId){ searchPromise=lookupByImdb(info.imdbId).then(function(showId){ if(showId) return loadShow(showId,season); return false; }); } else if(info.engTitle&&info.engTitle.length>=3){ searchPromise=searchAndLoad(info.engTitle,info,season); } else { searchPromise=Promise.resolve(false); }
        searchPromise.then(function(success){ if(success) return; var searchName=info.aka||info.chTitle; if(searchName) return searchAndLoad(searchName,info,season); return false; }).then(function(success){ if(!success){ if(!fallbackFromDouban(info)) showResult({notFound:true,reason:'TVmaze未收录'}); } }).catch(function(err){ console.error('[完结时间] error:',err); if(!fallbackFromDouban(info)) showResult({notFound:true,reason:'查询失败'}); });
    }
