...
(function () {
  ...
  var MODEL_QUOTA = (function () {
    ...
    function findModelSelector() {
      // Strategy 1: Find element containing model name (with or without "G " prefix)
      var modelPatterns = /MiMo|DeepSeek|Qwen|Grok|GLM|Muse|Nemotron|Free Models|Auto Router|OpenCode/i;
      ...
      for (var j = allEls.length - 1; j >= 0; j--) {
        ...
        // Must contain a model name but not be a container with lots of children
        if (modelPatterns.test(t) && t.length < 60) {
          // Verify it's a trigger element (not inside a dropdown list)
          var parent = el.parentElement;
          if (parent && parent.querySelector('[role="menu"], [role="listbox"], [data-slot]')) {
            // This element is inside a dropdown, skip
            continue;
          }
          return el;
        }
