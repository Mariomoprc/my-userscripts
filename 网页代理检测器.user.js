...
(function() {
    ...
    // ③ 显示模式切换（循环：小圆点 → 页面横幅 → 仅菜单）
    const modeLabels = { dot: '小圆点', banner: '页面横幅', menu: '仅菜单' };
    const modes = ['dot', 'banner', 'menu'];
    const currentModeIndex = modes.indexOf(CONFIG.displayMode);
    const nextMode = modes[(currentModeIndex + 1) % modes.length];

    GM_registerMenuCommand(`● ${modeLabels[CONFIG.displayMode]} → ${modeLabels[nextMode]}`, () => {
        CONFIG.displayMode = nextMode;
        GM_setValue('displayMode', nextMode);
        uiPrompt.removeElement();
        if (nextMode !== 'menu') init();
        tip(`显示模式 → ${modeLabels[nextMode]}`);
    });
